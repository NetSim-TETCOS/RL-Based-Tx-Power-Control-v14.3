...
# Create subplots for gNB powers
plt.figure(figsize=(10, 6 * num_gNBs))  # Adjust figure size based on number of gNBs

# Define colors for each gNB
colors = ['blue', 'green', 'red', 'purple', 'orange']  # Add more colors if needed

for i in range(num_gNBs):
    plt.subplot(num_gNBs, 1, i + 1)
    plt.plot(gnb_powers_plotting[i], label=f'gNB{i+1}', color=colors[i % len(colors)])
    plt.ylabel(f"Power (dBm)")
    plt.legend()
    
    # Only show xlabel on the bottom subplot
    if i == num_gNBs - 1:
        plt.xlabel("Iterations")
    else:
        plt.tick_params(labelbottom=False)
    
    # Only show title on the top subplot
    if i == 0:
